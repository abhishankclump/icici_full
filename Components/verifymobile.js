...
const Newpin = ({navigation}) => {
  ...
  if(value.length>5){
    navigation.navigate('changemobileone')
    }
  return (
    <CodeField
      ref={ref}
...
const Abhishank = ({navigation}) => {
  ...
  return (
    <View style={styles.container}>
      <Topbar title="Verify mobile number" navigation={navigation} />
    <ScrollView>
      <View style={{ elevation: 30, borderRadius: 15, backgroundColor: 'white', marginBottom:100 , marginTop: 10, marginHorizontal: 10, paddingVertical: 25, paddingHorizontal: 20 }}>
        <Text style={{ fontSize: 18, fontWeight: 'bold', color: 'black' }}>OTP has been sent to the registerd number *******180</Text>
